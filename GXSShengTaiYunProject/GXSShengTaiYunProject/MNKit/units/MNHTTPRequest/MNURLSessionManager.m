                            ...
                            error:(NSError *)error
{
    /**判断task是否有效*/
    if (!task) {
        NSLog(@"⚠️⚠️⚠️⚠️=请求task无效⚠️⚠️⚠️⚠️");
        return;
    }
    /**取出请求对象*/
    ...
    /**修改数据来源*/
    if ([request isKindOfClass:MNURLDataRequest.class]) ((MNURLDataRequest *)request).dataSource = MNURLDataSourceNetwork;
    /**处理数据, 回调结果*/
...
- (BOOL)loadDataWithRequest:(__kindof MNURLDataRequest *)request {
    /**读取缓存*/
    if (request.method == MNURLHTTPMethodGet && request.cachePolicy != MNURLDataCachePolicyNever) {
        id<NSCoding> cache = [self cacheForUrl:request.cacheForUrl timeoutInterval:(request.cachePolicy == MNURLDataCachePolicyUseTime ? request.cacheTimeInterval : 0.f)];
        if (cache) {
...
- (void)cancelRequest:(__kindof MNURLRequest *)request {
    ...
    if (request.isAllowsNetworkActivity && !request.isAllowsCancelCallback) [UIApplication closeNetworkActivityIndicating];
    [request.task cancel];
}
                       ...
                       completion:(void (^)(NSData *resumeData))completion
{
    ...
    [downloadTask cancelByProducingResumeData:^(NSData * _Nullable resumeData) {
        ...
        if (downloadRequest) {
            ...
            if (downloadRequest.isAllowsNetworkActivity && !request.isAllowsCancelCallback) [UIApplication closeNetworkActivityIndicating];
        }
