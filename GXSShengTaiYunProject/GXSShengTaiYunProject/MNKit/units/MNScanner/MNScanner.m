...
- (BOOL)isLighting {
    return (_device && _device.hasTorch && _device.torchMode == AVCaptureTorchModeOn);
}
...
- (BOOL)closeLighting {
    ...
    if (_device.hasTorch == NO) return YES;
    if (![_device isTorchModeSupported:AVCaptureTorchModeOff]) return NO;
