...
///指定播放的item
@property (nonatomic, strong) AVPlayerItem *pointItem;
@end
...
/// 补充方法 为滤镜写的扩展方法
- (instancetype)initWithURL:(NSURL *)URL playerItem:(AVPlayerItem *)playerItem
{
    if (URL) {
        self.pointItem = playerItem;
        return [self initWithURLs:@[URL]];
    }
    return [self init];
}
...
- (void)prepareToPlay {
    ...
    @synchronized (self) {
        ...
        if (self.pointItem != nil) {
            playerItem = self.pointItem;
        }
