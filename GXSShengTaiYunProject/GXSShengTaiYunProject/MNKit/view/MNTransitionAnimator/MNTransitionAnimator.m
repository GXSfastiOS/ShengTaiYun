...
static NSString * MNTransitionModalTouchAssociatedKey = @"mn.transition.modal.touch.associated";
...
+ (void)load {
    MNTransitionAnimatorSet = @[@"MNSlideTransitionAnimator",
                                        ...
                                        @"MNPushModalTransitionAnimator",
                                        @"MNDefaultModelTransitionAnimator",
                                        @"MNSheetTransitionAnimator",
                                        @"MNAlertTransitionAnimator",
                                        @"MNMenuTransitionAnimator",
                                        @"MN3DMenuTransitionAnimator",
                                        @"MN3DTransitionAnimator"];
}
...
- (void)restoreTabBarTransitionSnapshot {
    if (_fromView.tabBar_) [_fromView addSubview:_fromView.tabBar_];
}
...
- (UIView *)modalTouchView {
    return objc_getAssociatedObject(self, &MNTransitionModalTouchAssociatedKey);
}

- (void)setModalTouchView:(UIView *)modalTouchView {
    objc_setAssociatedObject(self, &MNTransitionModalTouchAssociatedKey, modalTouchView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)cleanTransitionShadow {
    self.layer.shadowPath = NULL;
    self.layer.shadowColor = NULL;
