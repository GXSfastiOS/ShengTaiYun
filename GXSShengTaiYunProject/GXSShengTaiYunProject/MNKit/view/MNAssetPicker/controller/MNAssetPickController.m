...
- (void)cameraController:(MNCameraController *)cameraController didFinishWithContents:(id)content {
    if ([content isKindOfClass:UIImage.class] && !self.configuration.isOriginalExporting && self.configuration.maxExportPixel > 0.f) {
        UIImage *image = [((UIImage *)content) resizingToPix:self.configuration.maxExportPixel];
...
- (void)imageCropController:(MNImageCropController *)controller didCroppingImage:(UIImage *)image {
    if (self.configuration.maxExportPixel > 0 && !self.configuration.isOriginalExporting) image = [image resizingToPix:self.configuration.maxExportPixel];
