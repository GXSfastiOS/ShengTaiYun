...
- (void)cameraController:(MNCameraController *)cameraController didFinishWithContents:(id)content {
    ...
    if ([self.configuration.delegate respondsToSelector:@selector(assetPicker:didFinishPickingAssets:)]) {
        if ([content isKindOfClass:UIImage.class] && !self.configuration.isOriginalExporting && self.configuration.maxExportPixel > 0.f) {
            content = [kTransform(UIImage *, content) resizingToPix:self.configuration.maxExportPixel];
        }
...
- (void)imageCropController:(MNImageCropController *)controller didCroppingImage:(UIImage *)image {
    if ([self.configuration.delegate respondsToSelector:@selector(assetPicker:didFinishPickingAssets:)]) {
        if (self.configuration.maxExportPixel > 0.f && !self.configuration.isOriginalExporting) image = [image resizingToPix:self.configuration.maxExportPixel];
