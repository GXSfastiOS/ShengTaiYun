...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        tailorHandler.pathColor = [UIColor colorWithHex:@"#999999"];//MNVideoTailorBlackColor;
        tailorHandler.normalColor =  [UIColor colorWithHex:@"#E2E2E2"];
        tailorHandler.highlightedColor =  [UIColor colorWithHex:@"#E2E2E2"];
...
- (void)setPointerProgress:(float)progress {
    CGPoint contentOffset       = self.scrollView.contentOffset;
    CGFloat centerX             = self.thumbnailView.width_mn * progress - MAX(contentOffset.x, 0.f) + self.scrollView.left_mn;
    if (!isnan(centerX)) {
        self.pointer.centerX_mn     = centerX;
