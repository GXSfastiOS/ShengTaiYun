...
+ (MNAsset *)assetWithContent:(id)content configuration:(MNAssetPickConfiguration *)configuration {
    ...
    if ([content isKindOfClass:UIImage.class]) {
        ...
        if (image.isAnimatedImage) {
            ...
            model->_thumbnail = [image.images.firstObject resizingToPix:MAX(model.renderSize.width, model.renderSize.height)];
        } else {
            model->_type = MNAssetTypePhoto;
            model->_thumbnail = [image resizingToPix:MAX(model.renderSize.width, model.renderSize.height)];
        }
        ...
        model->_durationString = [NSDate playTimeStringWithInterval:@([MNAssetExporter exportDurationWithMediaAtPath:content])];
        if (configuration && configuration.isAllowsDisplayFileSize) {
            NSNumber *videoSize;
        ...
        if (@available(iOS 9.1, *)) {
            ...
            model->_thumbnail = [[UIImage imageWithContentsOfFile:imageURL.path] resizingToPix:MAX(model.renderSize.width, model.renderSize.height)];
