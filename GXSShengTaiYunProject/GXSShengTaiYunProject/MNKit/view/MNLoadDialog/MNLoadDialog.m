...
@property (nonatomic) MNLoadDialogStyle style;
...
+ (void)load {
    ...
    MNLoadDialogPool = @[@"MNActivityDialog",
                                    @"MNShapeDialog",
                                    @"MNRotatedDialog",
                                    @"MNBallDialog",
                                    @"MNDotDialog",
...
+ (instancetype)loadDialogWithStyle:(MNLoadDialogStyle)style message:(NSString *)message {
    ...
    MNLoadDialog *dialog = [[NSClassFromString([MNLoadDialogPool objectAtIndex:style]) alloc] initWithMessage:message];
    dialog.style = style;
    return dialog;
}
