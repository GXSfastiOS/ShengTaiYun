...
- (void)setAllowsEditing:(BOOL)allowsEditing {
    ...
    if (allowsEditing) {
        ...
        recognizer.cell_edit_label = MNTableViewCellRecognizerKey;
        ...
        [self.contentView.gestureRecognizers enumerateObjectsUsingBlock:^(__kindof UIGestureRecognizer * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj isKindOfClass:UIPanGestureRecognizer.class] && [obj.cell_edit_label isEqualToString:MNTableViewCellRecognizerKey]) {
                recognizer = obj;
            }
        }];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context  {
    if (_editView && [keyPath isEqualToString:MNTableViewCellObservedKeyPath]) {
...
- (BOOL)gestureRecognizerShouldBegin:(UIPanGestureRecognizer *)recognizer {
    ...
    if (![recognizer.cell_edit_label isEqualToString:MNTableViewCellRecognizerKey]) return NO;
    /// 判断方向
    CGPoint translation = [recognizer translationInView:recognizer.view];
    BOOL shouldBegin = fabs(translation.y) <= fabs(translation.x);
    if (!shouldBegin) return NO;
    // 询问代理是否需要侧滑
    if ([_editDelegate respondsToSelector:@selector(tableViewCell:canEditRowAtIndexPath:)]) {
        shouldBegin = [_editDelegate tableViewCell:self canEditRowAtIndexPath:self.index_path];
    }
    if (!shouldBegin) return NO;
    ...
    /// 无响应编辑按钮不可编辑
    if (![_editDelegate respondsToSelector:@selector(tableViewCell:editingActionsForRowAtIndexPath:)]) return NO;
    /// 获取编辑按钮
    NSArray <MNTableViewCellEditAction *>*actions = [_editDelegate tableViewCell:self editingActionsForRowAtIndexPath:self.index_path];
...
- (void)tableViewCellEditView:(MNTableViewCellEditView *)editView didClickAction:(MNTableViewCellEditAction *)action {
    ...
    if ([_editDelegate respondsToSelector:@selector(tableViewCell:commitEditingAction:forRowAtIndexPath:)]) {
        UIView *nextEditView = [_editDelegate tableViewCell:self commitEditingAction:action forRowAtIndexPath:self.index_path];
        if (nextEditView) {
            /// 设置下一步编辑视图
...
- (void)setFailToGestureRecognizer:(UIGestureRecognizer *)failToGestureRecognizer {
    ...
    [self.contentView.gestureRecognizers enumerateObjectsUsingBlock:^(__kindof UIGestureRecognizer * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj.cell_edit_label isEqualToString:MNTableViewCellRecognizerKey]) {
            recognizer = obj;
            *stop = YES;
        }
    }];
...
@implementation UITableView (MNCellEditing)
- (BOOL)isEdit {
...
static NSString * const UIGestureRecognizerLabelKey = @"com.mn.gesture.recognizer.label.key";
@implementation UIGestureRecognizer (MNCellEditing)

- (void)setCell_edit_label:(NSString *)cell_edit_label {
    objc_setAssociatedObject(self, &UIGestureRecognizerLabelKey, cell_edit_label, OBJC_ASSOCIATION_COPY);
}

- (NSString *)cell_edit_label {
    return objc_getAssociatedObject(self, &UIGestureRecognizerLabelKey);
}
