...
+ (instancetype)menuWithAlignment:(MNMenuAlignment)alignment configuration:(MNMenuCreatedHandler)configurationHandler titles:(NSString *)title,...NS_REQUIRES_NIL_TERMINATION {
    if (!title) return nil;
    ...
    va_end(args);
    return [[self alloc] initWithTitles:titles.copy alignment:alignment configuration:configurationHandler];
}

- (instancetype)initWithTitles:(NSArray <NSString *>*)titles alignment:(MNMenuAlignment)alignment configuration:(MNMenuCreatedHandler)configurationHandler {
    ...
    [titles enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (configurationHandler) configurationHandler(self, button);
    }];
    ...
    clickedHandler:(MNMenuClickedHandler)clickedHandler
{
    ...
    if (clickedHandler) self.itemClickedHandler = clickedHandler;
    if (self.showHandler) {
...
+ (void)close {
    ...
    menuView.itemClickedHandler = nil;
