...
#ifdef ThemeColor
#define MNEmptyButtonColor  ThemeColor
#else
#define MNEmptyButtonColor  [UIColor colorWithRed:0.f/255.f green:122.f/255.f blue:254.f/255.f alpha:1.f]
#endif
...
- (void)createView {
    ...
    [button setTitleColor:MNEmptyButtonColor forState:UIControlStateNormal];
    [button.titleLabel setFont:UIFontRegular(MNEmptyViewFontSize)];
    button.layer.cornerRadius = 5.f;
    button.layer.borderColor = [MNEmptyButtonColor CGColor];
...
- (void)layoutSubviews {
    ...
    CGFloat maxW = self.width_mn - MNEmptyViewContentMargin*2.f;
    
    _label.size_mn = [NSString boundingSizeWithString:_label.text size:CGSizeMake(maxW, CGFLOAT_MAX) attributes:@{NSFontAttributeName:_label.font}];
...
- (void)setButtonTitle:(NSString *)buttonTitle {
    if (buttonTitle.length <= 0) {
        ...
        CGSize size = [NSString stringSize:buttonTitle font:UIFontRegular(MNEmptyViewFontSize)];
