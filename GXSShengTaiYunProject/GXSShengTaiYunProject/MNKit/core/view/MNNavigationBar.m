...
- (UIView *)createLeftItem {
    ...
    if ([_delegate respondsToSelector:@selector(navigationBarShouldDrawBackBarItem)] && [_delegate navigationBarShouldDrawBackBarItem]) {
        UIColor *backColor;
        if ([_delegate respondsToSelector:@selector(navigationBarLeftBarItemColor)]) {
            backColor = [_delegate navigationBarLeftBarItemColor];
        }
        else {
            backColor = [UIColor darkTextColor];
        }
        UIImage *leftItemImage = UIImageWithUnicode(MNFontUnicodeBack, backColor, kNavItemSize);
