...
- (instancetype)initWithURL:(NSURL *)URL {
    return [self initWithUrl:URL.isFileURL ? URL.path : URL.absoluteString];
}
...
- (void)navigationBarLeftBarItemTouchUpInside:(UIView *)leftBarItem {
    if (leftBarItem.tag == MNWebViewBackViewTag) {
        ...
        }else if(self.VIPInfo){
//            [_webView evaluateJavaScript:@"go_back();" completionHandler:nil];
...
- (void)loadRequest:(id)req {
    if (!req) return;
    NSURLRequest *request = [self handRequest:req];
    if (!request) return;
...
- (void)webView:(WKWebView *)webView didFailProvisionalNavigation:(null_unspecified WKNavigation *)navigation withError:(NSError *)error {
    if (error.code == NSURLErrorCancelled || (error.code == 102 && [error.domain isEqual:@"WebKitErrorDomain"])) {
        if (error.code == NSURLErrorCancelled && _reloadButton) [_reloadButton.layer pauseAnimation];
        return;
    }
    if (_reloadButton) [_reloadButton.layer pauseAnimation];
...
#pragma mark - dealloc
- (void)dealloc {
    if (_reloadButton) [_reloadButton.layer resetAnimation];
    if (_reloadButton) [_reloadButton.layer removeAllAnimations];
    [_webView safelyRemoveObserver:self forKeyPath:@"title"];
    [_webView safelyRemoveObserver:self forKeyPath:@"estimatedProgress"];
