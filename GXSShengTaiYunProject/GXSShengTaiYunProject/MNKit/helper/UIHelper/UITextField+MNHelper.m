...
//键盘样式
- (void)wzm_addInputAccessoryView {
    
    UIVisualEffectView *toolView = [[UIVisualEffectView alloc] initWithEffect:[UIBlurEffect effectWithStyle:UIBlurEffectStyleLight]];
    toolView.frame = CGRectMake(0, 0, MN_SCREEN_WIDTH, 40);
    toolView.backgroundColor = [UIColor colorWithWhite:.9 alpha:.9];
    
    NSArray *titles = @[@".",@"·",@"-"];
    CGFloat minX = (MN_SCREEN_WIDTH-60.0*3)/2.0;
    for (NSInteger i = 0; i < titles.count; i ++) {
        UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(minX+i*60.0, 0.0, 60.0, 40.0)];
        btn.tag = i;
        btn.titleLabel.font = [UIFont systemFontOfSize:20.0];
        [btn setTitle:titles[i] forState:UIControlStateNormal];
        [btn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [btn addTarget:self action:@selector(wzmBtnClick:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)wzmBtnClick:(UIButton *)btn {
    NSArray *titles = @[@".",@"·",@"-"];
    NSString *text = [titles objectAtIndex:btn.tag];
